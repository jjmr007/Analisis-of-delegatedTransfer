...
A diferencia de esto, **_delegatedTransfer_** es una función segura de delegación de fondos y al mismo tiempo una asignación efectiva de fondos hacia cualquier entidad (sea contrato o cuenta normal), y por ende puede ser invocada desde un contrato, a través de funciones que pueden invocar otras funciones, de cualquier otra cantidad de contratos que se necesiten **_en una sola transacción_**. Lo que hace posible que el usuario final, ni siquiera tenga que lidiar con la compra de una *criptomoneda* llamada Ethereum y adicionalmente lo que convierte potencialmente a la función **_delegatedTransfer_** en un sustituto idóneo para *approve* y *transferFrom*, dejando obsoletas estas funciones en el estándar ERC20.<br><br>

**iii.- ¿En Qué Casos Resulta Útil la Función _delegatedTransfer_?**. En este corto análisis me quiero referir a dos casos interesantes en que una moneda estable puede recibir un gran provecho:
...
Proveer fondos para una campaña de colecta para la promoción de un proyecto, puede realizarse mediante monedas estables a través de la mediación de un contrato. Este contrato que llamaremos el "ICO proyecto", puede ejecutar una serie de promesas, al momento (es decir, en la misma transacción) de recibirse los fondos de un cierto financiamiento, de un modo simple para el usuario y abaratando costos de transacción.
...
Es importante recordar que en la figura 2, el relayer es asumido por un servidor o algún tipo de back-end generalmente centralizado. Si lo que se busca es un diseño más distribuido, el back-end puede ser sustituido por una blockchain privada bajo un paradigma [POA](https://wiki.parity.io/Proof-of-Authority-Chains).
...
En realidad, esta ultima función sólo haría falta ejecutarla una sola vez. Después de ello, el mezclador queda autorizado a efectuar movilizaciones delegadas de fondos de este token Dai, para lo que la función normal "**_deposit_**" puede ser invocada sin tener que preocuparnos más por "**_approve_**".

**iv.- ¿Cómo Generar una Firma (v, r, s)?**. La solución que nos presenta **_delegatedTransfer_** viene con algunas complicaciones. Si bien es cierto que le puede ahorrar al usuario una transacción, en casos en que este usuario no posee ethers y no va a costear la gasolina de ninguna transacción, se deberá depender de alguna plataforma back-end. 
